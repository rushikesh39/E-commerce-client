...
const Computer = () => {
  ...
  const products=product.filter(item=>item.product_category_tree[0]==="laptop")
  console.log("laptops", products);
  ...
  useEffect(() => {
    if(products.length<20){
      const fetchProducts = async () => {
        try {
          const response = await axios.get("https://ecommerce-server-hpa9.onrender.com/product/computer");
