...
const Jwelary = () => {
  ...
  console.log("jwelary product",product)
  const products=product.filter(item=>item.product_category_tree[0].includes("Jewellery"))
  console.log("jewellery", products);
  ...
  useEffect(() => {
    if(products.length<20){
      const fetchProducts = async () => {
        try {
          const response = await axios.get("https://ecommerce-server-hpa9.onrender.com/product/jewellery");
