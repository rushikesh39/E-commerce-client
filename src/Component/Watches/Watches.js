...
const Watches = () => {
  ...
  const products=product.filter(item=>item.product_category_tree[0].includes("Watches"))
  ...
  useEffect(() => {
    if(products.length<20){
      const fetchProducts = async () => {
        try {
          const response = await axios.get("https://ecommerce-server-hpa9.onrender.com/product/watches");
...
<div className="mobile-container">
      {category && <h2>{category}'s Watches </h2>}
        {categoryProduct &&
